...
const { encryptPassword, verifyPassword } = require("../scripts/encrypt");
...
module.exports = {
  ...
  verifyAccount: function (req, res) {
    ...
    const { username, password } = JSON.parse(req.header("Authorization"));
    accountDb
      .findOne({
        username,
      })
  ...
  verifyInitialization: function (req, res) {
    ...
    accountDb.find({ type: "Admin" }).then((accounts) => {
      if (!accounts.length) {
        adminDb
          .create({
  ...
  seedDefault: function (req, res) {
    ...
    fs.writeFile(filePath, "Hello World!", "utf8", (err) => {
      ...
      // 1
      fileDb.create(fileObj).then((newF) => {
        ...
        // 2
        announcementDb.create(annDocs).then((newDocs) => {
          ...
          // 3
          subjectDb.create(subjectDoc).then((newS) => {
            ...
            promises.push(
              announcementDb.findOneAndUpdate(
                { _id: privateAnn._id },
                privateAnn
              )
            );
